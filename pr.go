...
func CreatePullRequest() error {
  ...
  if !(len(sourceValues) == 1 || len(sourceValues) == 2){
    return fmt.Errorf("Source repo must be in the format [owner:]branch got %v", sourceRepo)
  }
  destBranchValues := strings.Split(destRepo, ":")
  if len(destBranchValues) != 2 {
    return fmt.Errorf("Source repo must be in the format [owner:]branch got %v", destRepo)
  }
  destValues := strings.Split(destBranchValues[0], "/")
  if len(destValues) != 2 {
    return fmt.Errorf("Destination repo must be in the format [owner:]branch got %v", destRepo)
  }
...
func PullRequestSuccess(resp *http.Response) error {
  ...
  fmt.Printf("Created Pull Reuest: %s", respContent.URL)
  return nil
}
